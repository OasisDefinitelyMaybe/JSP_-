...
<script type="text/javascript">
   ...
   */
    function validateForm(form) {
    	  ...
    	  // 체크로직 모두 통화했다면 서버에 요청, 체크로직을 통과하지 못했다면 메세지 처리
    	  if(form.kor.value == ''){
    		  alert('국어점수를 입력해주세요');
    		  testForm.kor.focus();
    		  return false;
...
<!-- form에 action을 주지않은 경우, 동일한 페이지를 다시 요청합니다. -->
<form name="testForm">
   ...
   <!-- 폼에 버튼이 하나만 있다면 이 버튼은 서브밋 버튼이 됩니다.
        서브밋 버튼은 폼을 전송하는 기본이벤트가 정의되어있습니다.
        
        유효성검사(validation체크)를 진행하기 위해서는 
        기본이벤트를 제거하는 작업을 해야합니다. 
     --> 
...
<c:if test="${ not ( empty param.kor
	...
	평균점수 : ${(param.kor + param.eng + param.math) /3 }
	
	<c:set var="avg" value="${(param.kor + param.eng + param.math) /3 }"/>
    <p>평균 : ${avg }</p>
    
    <h2>국, 영, 수 점수를 입력하면 평균을 내어 학점을 출력</h2>
    <p> >=90 : A학점, >=80 : B학점, >=70 : C학점, >= : D학점</p>
    ${param.name }님의 학점은
    <c:choose>
