 ...
 */
@WebServlet("/06session/ex/servletEx/loginProcess")
public class LoginController extends HttpServlet {
     ...
     *     2-2. 사용자가 존재하지 않으면 이전 페이지로 넘어가서 오류 메세지를 출력
     */
...
//			dao.close();
			...
			// list값을 화면으로 전달할 수 없어
			//request.getRequestDispatcher("board.jsp").forward(request, response);
			// response.sendRedirect("board.jsp");  
			//      --> sendRedirect를 이용할 경우, request영역이 공유가 되지 않기 때문에 list값을 화면으로 전달할 수 없음. 
