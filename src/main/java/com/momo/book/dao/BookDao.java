...
public class BookDao extends DBConnPool {
	...
	public List<BookDto> getList(Criteria cri) {
		...
		String sql = " select *\r\n"
					+ "        from book\r\n"
					+ "        -- 최신게시물을 먼저 조회하기 위해서 정렬합니다\r\n"
					+ where
					+ "        order by no desc";
		
		
		  try {
			// pageingQuery를 이용시 페이지 처리를 위한 파라미터 세팅을 해줘야 함
			  ...
			  // 결과집합으로부터 도서의 정보를 가지고와서 dto에 저장 후 리스트에 담아준다.
		      while(rs.next()) {
	...
	public int regBook(BookDto dto) {
		...
		try {
			...
			// select 문항은 결과집합을 반환 : rs = pstmt.executeQuery()
			// insert, delete, update의 반환타입은 몇건이 처리되었는지 결과
